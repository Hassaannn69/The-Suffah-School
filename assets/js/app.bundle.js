...
var App = (() => {
  ...
  var require_app = __commonJS({
    "assets/js/app.js"(exports) {
      ...
      var SIDEBAR_COLLAPSED_KEY = "suffah_sidebar_collapsed";
      var SIDEBAR_MODE_KEY = "suffah_sidebar_mode";
      var currentSidebarMode = localStorage.getItem(SIDEBAR_MODE_KEY) || "toggle";
      ...
      function initApp() {
        ...
        `;
          ...
          try {
            ...
            yield loadModule(initialModule);
              ...
              item.submenu.forEach((subitem) => {
                if (subitem.roles.includes(currentRole)) {
                  ...
                  sublink.addEventListener("click", (e) => {
                    e.preventDefault();
                    if (typeof window.expandSidebar === "function")
      ...
      function loadHeaderVersion() {
        return __async(this, null, function* () {
          ...
          try {
            const response = yield fetch(`version.json?t=${Date.now()}`, { cache: "no-store" });
            if (!response.ok)
              throw new Error("Failed to fetch version");
            const data = yield response.json();
            if (data.version && headerVersionEl)
              headerVersionEl.textContent = data.version;
            ...
            if (headerVersionEl)
              headerVersionEl.textContent = "1.0.0";
            if (headerLastUpdatedEl)
              headerLastUpdatedEl.textContent = "unknown";
          }
        });
      }
      ...
      function loadModule(moduleId) {
        ...
        `;
            try {
              ...
              const menuItem = menuItems.find((m) => m.id === moduleId) || menuItems.flatMap((m) => m.submenu || []).find((m) => m.id === moduleId);
              ...
              if (moduleId === "dashboard") {
                if (currentRole === "student")
                  ...
                  actualModuleId = "teacher-dashboard";
              }
      ...
      window.initializePremiumDropdowns = function(container = document) {
        ...
        selects.forEach((select) => {
          ...
          const updateTriggerText = () => {
            ...
            trigger.textContent = selected ? selected.textContent : ((_a = select.options[0]) == null ? void 0 : _a.textContent) || "Select...";
          };
          updateTriggerText();
      ...
      if (!window.globalSelectObserverInitialized) {
        ...
        const globalSelectObserver = new MutationObserver((mutations) => {
          ...
          debounceTimer = setTimeout(() => {
            ...
            for (const mutation of mutations) {
              if (mutation.addedNodes.length) {
                for (const node of mutation.addedNodes) {
                  if (node.nodeType === 1 && (node.tagName === "SELECT" || ((_a = node.querySelector) == null ? void 0 : _a.call(node, "select")))) {
                    shouldInit = true;
                    break;
                  }
                }
            ...
            if (shouldInit)
              window.initializePremiumDropdowns(mainContent || document);
          }, 100);
        });
      ...
      function startUpdateCheck() {
        ...
        function check() {
          ...
          fetch(`version.json?t=${Date.now()}`, { cache: "no-store" }).then((r) => r.ok ? r.json() : null).then((data) => {
            ...
            if (!serverVersion || serverVersion === "undefined")
              return;
